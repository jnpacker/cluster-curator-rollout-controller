...
// Distributor handles the distribution of labels and versions across synthetic clusters
...
// DistributeVersions applies base version distribution to all clusters
func (d *Distributor) DistributeVersions(versions []models.VersionDef) {
	if len(versions) == 0 {
		return
	}

	// Calculate distribution counts
	distribution := calculateVersionDistribution(versions, d.totalClusters)

	// Apply versions to clusters
	clusterIdx := 0
	for versionIdx, count := range distribution {
		version := versions[versionIdx].Version
		for i := 0; i < count && clusterIdx < d.totalClusters; i++ {
			d.clusters[clusterIdx].BaseVersion = version
			clusterIdx++
		}
	}
}

// calculateVersionDistribution calculates how many clusters should get each version
func calculateVersionDistribution(versions []models.VersionDef, totalClusters int) []int {
	distribution := make([]int, len(versions))
	...
	for _, i := range specifiedIndices {
		count := (totalClusters * versions[i].Percentage) / 100
